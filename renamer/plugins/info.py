...
def user_info(user):
    ...
    text += f"\n\n**❌ Is Deleted :** True" if user.is_deleted else "False"
    text += f"\n\n**🤖 Is Bot :** True" if user.is_bot else "False"
    text += f"\n\n**✅ Is Verified :** True" if user.is_verified else "False"
    text += f"\n\n**✖️ Is Restricted :** True" if user.is_verified else "False"
    text += f"\n\n**💨 Is Scam :** True" if user.is_scam else "False"
    text += f"\n\n**👺 Is Fake :** True" if user.is_fake else "False"
    text += f"\n\n**🤔 Is Support :** True" if user.is_support else "False"
    text += (
...
def chat_info(chat):
    ...
    text += f"\n\n**Is Verified :** True" if chat.is_verified else "False"
    text += f"\n\n**Is Restricted :** True" if chat.is_verified else "False"
    text += f"\n\n**Is Creator :** True" if chat.is_creator else "False"
    text += f"\n\n**Is Scam :** True" if chat.is_scam else "False"
    text += f"\n\n**Is Fake :** True" if chat.is_fake else "False"
    return text
